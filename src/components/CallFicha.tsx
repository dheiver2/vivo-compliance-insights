    ...
    <>
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <CardDescription>
                  Protocolo <span className="font-mono text-primary">{call.protocol}</span> ·{" "}
                  {call.topic} · {new Date(call.createdAt).toLocaleString("pt-BR")}
                </CardDescription>
                <div className="mt-1.5 flex items-center gap-1.5 text-sm">
                  ...
                  <Link
                    to="/team/$agentName"
                    params={{ agentName: call.agentName }}
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                ...
                <div
                  className={`text-3xl font-display font-bold ${scoreCls(call.scoreCompliance)}`}
                >
                  {call.scoreCompliance}%
                </div>
              </div>
              <div className="text-center">
                <div className="text-[10px] uppercase text-muted-foreground">Qualidade</div>
                <div className={`text-3xl font-display font-bold ${scoreCls(call.scoreQuality)}`}>
                  {call.scoreQuality}%
                </div>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Resumo do caso e a resposta de cada item do script de monitoria. Por privacidade (LGPD),
            a transcrição completa não é armazenada.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <pre className="text-sm whitespace-pre-wrap font-sans text-foreground/90 bg-secondary/40 rounded-lg p-4 border border-border/60 max-h-[400px] overflow-y-auto">
            {call.transcript}
          </pre>
        </CardContent>
      </Card>
