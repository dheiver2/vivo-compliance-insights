...
import { ArrowLeft, Loader2, Search, User, AudioLines, FileText, TrendingDown, Lightbulb } from "lucide-react";
      ...
      {data === null && (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center text-center py-24 text-muted-foreground">
            ...
            <p className="text-sm mt-1">Não há ligações atribuídas a <span className="font-medium">{agentName}</span>.</p>
            <Link to="/team" className="mt-6 inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
              Ver toda a equipe
            </Link>
          </CardContent>
        </Card>
        ...
        <>
              ...
              <div className="flex items-start justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <CardDescription>
                      {data.performance.calls} ligação(ões) auditada(s) · última em {new Date(data.performance.lastCallAt).toLocaleString("pt-BR")}
                    </CardDescription>
                  </div>
                </div>
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    ...
                    <div className={`text-3xl font-display font-bold ${scoreCls(data.performance.avgCompliance)}`}>{data.performance.avgCompliance}%</div>
                  </div>
                  <div className="text-center">
                    <div className="text-[10px] uppercase text-muted-foreground">Qualidade</div>
                    <div className={`text-3xl font-display font-bold ${scoreCls(data.performance.avgQuality)}`}>{data.performance.avgQuality}%</div>
                  </div>
                </div>
              </div>
            </CardHeader>
                ...
                <div className="flex items-start gap-3 rounded-lg border border-warning/40 bg-warning/5 p-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      O ponto mais fraco é <span className="font-medium text-foreground">{data.performance.weakestItem.label}</span> (média {data.performance.weakestItem.score}%). Priorize feedback nesse critério.
                    </p>
                  </div>
                </div>
              )}
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={240}>
                  <LineChart data={data.trend}>
                    ...
                    <Line type="monotone" dataKey="compliance" name="Compliance" stroke="oklch(0.42 0.22 310)" strokeWidth={3} dot={{ r: 4 }} />
                    <Line type="monotone" dataKey="quality" name="Qualidade" stroke="oklch(0.62 0.16 155)" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Desempenho regulatório agregado das ligações deste atendente</CardDescription>
            </CardHeader>
                ...
                <div key={item.label}>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="font-medium flex items-center gap-1.5">
                      {data.performance.weakestItem?.label === item.label && <TrendingDown className="h-3.5 w-3.5 text-destructive" />}
                      {item.label}
                    </span>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{data.calls.length} registro(s) — mais recentes primeiro</CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              {data.calls.map((c) => (
                ...
                >
                  {c.origin === "audio" ? <AudioLines className="h-4 w-4 text-primary flex-shrink-0" /> : <FileText className="h-4 w-4 text-primary flex-shrink-0" />}
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium truncate">{c.label}</div>
                    <div className="text-xs text-muted-foreground">{c.topic} · {new Date(c.createdAt).toLocaleString("pt-BR")}</div>
                  </div>
                  <span className={`text-sm font-semibold ${scoreCls(c.scoreCompliance)}`}>{c.scoreCompliance}%</span>
