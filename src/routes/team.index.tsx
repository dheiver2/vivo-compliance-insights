...
import { Users, Loader2, Sparkles, TrendingDown, Smile, Meh, Frown, ChevronRight, Trophy } from "lucide-react";

export const Route = createFileRoute("/team/")({
  head: () => ({ meta: [{ title: "Equipe · VoiceAudit" }, { name: "description", content: "Desempenho de compliance e qualidade por atendente." }] }),
  component: TeamPage,
});
...
function SentimentBar({ s }: { s: AgentPerformance["sentiment"] }) {
  ...
  return (
    <div className="flex items-center gap-3 text-xs">
      <span className="flex items-center gap-1 text-success"><Smile className="h-3.5 w-3.5" />{s.positivo}</span>
      <span className="flex items-center gap-1 text-warning-foreground"><Meh className="h-3.5 w-3.5" />{s.neutro}</span>
      <span className="flex items-center gap-1 text-destructive"><Frown className="h-3.5 w-3.5" />{s.negativo}</span>
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">Ranking de atendentes por score de compliance — derivado das ligações auditadas</p>
      </header>
      ...
      {data && data.length === 0 && (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center text-center py-24 text-muted-foreground">
            ...
            <p className="text-sm mt-1">Informe o atendente ao analisar uma ligação para alimentar o ranking.</p>
            ...
            >
              <Card className="hover:shadow-[var(--shadow-elegant)] hover:border-primary/40 transition-all cursor-pointer">
                <CardContent className="flex flex-wrap items-center gap-4 py-4">
                  <div className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full text-sm font-bold ${rankMedal(i)}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {a.calls} ligação(ões) · {a.criticalCount} crítica(s) · {a.approvedCount} aprovada(s)
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-center">
                      ...
                      <div className={`text-xl font-display font-bold ${scoreCls(a.avgCompliance)}`}>{a.avgCompliance}%</div>
                    </div>
                    <div className="text-center">
                      <div className="text-[10px] uppercase text-muted-foreground">Qualidade</div>
                      <div className={`text-xl font-display font-bold ${scoreCls(a.avgQuality)}`}>{a.avgQuality}%</div>
                    </div>
                    <StatusBadge status={a.avgCompliance >= 85 ? "approved" : a.avgCompliance >= 70 ? "warning" : "critical"} />
                  </div>
                  ...
                  <div className="w-full lg:w-auto lg:min-w-[240px] space-y-2">
                    ...
                    {a.weakestItem && (
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span className="truncate">Foco: {a.weakestItem.label} ({a.weakestItem.score}%)</span>
                      </div>
                    )}
                  </div>
